...
1. Prepare the boot components and store them in the same directory. 

   - Boot components are generated by PetaLinux. For Versal devices, they are the following files:
      - bl31.elf
      - u-boot.elf
      - system.dtb
   - These files are the source of creating final BOOT.BIN during application packaging. When creating the platform, these files will be copied to the platform output directory. 
   - PetaLinux generates them in **<your_petalinux_dir>/images/linux** directory. 
   - Please find these files and copy them into **step3_pfm/boot** for easy understanding. 
   - Please note that in our fast track script, they were kept in original place to make the script simpler and easier to maintain. 
