...
The control path needs a DFX Decoupler IP. It needs to work with the Register Slice inside Vitis Region to ensure shortest path across the boundary. The SmartConnect in Vitis Region can export control interfaces for the platform. V++ linker would connect PL kernel control interface to the SmartConnect.
...
7. Configure noc_ddr using Block Automation
   ...
   5. Enable LOW_1 memory region
   6. Click OK
   7. The block automation helps to setup DDR memory configuration, adds DDR clock and data pin connections and adds a noc_clk_gen block for simulation.
   8. Remove the generated noc_clk_gen block because will will setup the clock and reset manually later. This block is only needed if your design doesn't use CIPS generated PL clocks. 

8. Configure noc_ddr to maximize the internal interface bandwidth. 
   1. Double click noc_ddr
   2. Go to **General** tab, update Number of **Memory Controller Ports** to **4**.
...
4. Connect clock and reset blocks
   ...
   4. Connect the following ports manually. If the signal crosses the VitisRegion boundary, rename the signal with VitisRegion Boundary Net Name
      ...
      VitisRegion/IsoReset | ext_reset_in | /clk_wizard_1/clk_out1 | 
      VitisRegion/IsoReset | slowest_sync_clk | /IsoReset1/peripheral_resetn | 
...
6. Add AXI Register Slice blocks
   ...
   5. Double click IsoRegStatic
   6. In Settings tab, change all settings to Manual and set according to the following table because we wish the AXI interface of Vitis Region could be fixed.
...
5. Convert the design to enable DFX
   1. Select menu **Tools** -> **Dynamic Function eXchange Wizard**
...
8. Add Pblock constraints for BDC
   ...
   3. Select **Add Files** and navigate to pblock.xdc. pblock.xdc is located in the source directory.
   4. Click **Finish**.
...
3. Setup platform interfaces
   ...
   2. Setup the AXI port for memory
   ...
   3. Setup the AXI interfaces for controlling kernels
