...
As may know Petalinux project need XSA file as the input which describes the hardware information. If you have Vivado project and export the XSA file you can use it as your project input. This time we will download and leverage the base platform of ZCU104 board form [Xilinx website download page](https://www.xilinx.com/support/download.html). Also you can download other platform or board from that page according to your need. So first we will download the base platform.
...
2. Create a PetaLinux project named **zcu104_petalinux** and configure the hw option with the XSA file:
   ...
   cd zcu104_petalinux
   petalinux-config --get-hw-description=xilinx_zcu104_base_202210_1/hw/hw.xsa  # After you extract the platform you can find hw.xsa or hw_emu.xsa under <xilinx_zcu104_base_202210_1> directory. if you want to do emulation you cna choose hw_emu.xsa 
   ```
...
3. A petalinux-config menu would be launched, Set to use ZCU104 device tree in this configuration window.
   ...
   Note: 
     ...
     - If you are using a Xilinx development board it is recommended to modify the machine name so that the board configurations would be involved in the DTS auto-generation. You can check the below table for the name of the corresponding board.
   ...
   Below I list the common platform board and its machine name for your reference :

      | Board  | Machine name                     |
      | ------ | -------------------------------- |
      | zcu104 | zcu104-revc                      |
      | vck190 | versal-vck190-reva-x-ebm-02-reva |
      | vmk180 | versal-vmk180-reva-x-ebm-03-reva |
      | zc102  | zcu102-reva                      |
