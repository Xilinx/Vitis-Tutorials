...
/**
 * @brief Run DPU Task for AlexNet //DB
 *
 * @param taskAlexNet - pointer to AlexNet Task  //DB
 *
 * @return none
 */
void runAlexNet(vart::Runner* runner) {  //DB
  /* Mean value for AlexNet specified in Caffe prototxt */
  ...
  for (unsigned int n = 0; n < images.size(); n += batchSize) {
    ...
    for (unsigned int i = 0; i < runSize; i++) {
      ...
      /* Display the impage */
      cv::imshow("Classification of AlexNet", imageList[i]);
      cv::waitKey(1); //(10000);
    }
    imageList.clear();
    inputs.clear();
...
/**
 * @brief Entry for running AlexNet neural network
 *
 * @note Runner APIs prefixed with "dpu" are used to easily program &
 *       deploy AlexNet on DPU platform.
 *
 */
int main(int argc, char* argv[]) {
  // Check args
  if (argc != 2) {
    cout << "Usage of video analysis demo: ./AlexNet path(for json file)"
         << endl;
  ...
  CHECK_EQ(subgraph.size(), 1u)
      << "AlexNet should have one and only one dpu subgraph.";
