...
const string baseImagePath = "./src/img_test/"; //DB
...
/**
 * @brief Run DPU Task for FCN8 //DB
 *
 * @param taskFCN8 - pointer to FCN8 Task  //DB
 *
 * @return none
 */
void runFCN8(vart::Runner* runner) {  //DB
...
//  //  float mean[3] = {104, 107, 123}; // DB
  ...
  for (unsigned int n = 0; n < images.size(); n += batchSize) {
    ...
    for (unsigned int i = 0; i < runSize; i++) {
      ...
      /* Display the impage */
      cv::imshow("Semantic Segmentation with FCN8", imageList[i]);
      cv::waitKey(10000);
    }
    imageList.clear();
    inputs.clear();
...
int main(int argc, char* argv[]) {
  // Check args
  if (argc != 2) {
    cout << "Usage: ./fcn8 path_for_elf_file" << endl;
    return -1;
  }
  auto graph = xir::Graph::deserialize(argv[1]);
  auto subgraph = get_dpu_subgraph(graph.get());
  CHECK_EQ(subgraph.size(), 1u)
      << "FCN8 should have one and only one dpu subgraph.";
