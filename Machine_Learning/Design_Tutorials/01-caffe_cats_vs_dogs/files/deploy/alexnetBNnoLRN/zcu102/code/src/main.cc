...
// based on  Vitis AI 1.3 VART resnet50 demo code
    ...
    for (unsigned int i = 0; i < runSize; i++) {
      ...
      for (int h = 0; h < inHeight; h++) {
        for (int w = 0; w < inWidth; w++) {
          for (int c = 0; c < 3; c++) {
            imageInputs[i * inSize + h * inWidth * 3 + w * 3 + c] = float(image2.at<Vec3b>(h, w)[c]) - mean[c];
          }
        }
      }
...
int main(int argc, char* argv[])
{
  ...
  if (argc != 2) {
    ...
    cout << "Usage: run_cnn elf_pathName" << endl;
    return -1;
  }
