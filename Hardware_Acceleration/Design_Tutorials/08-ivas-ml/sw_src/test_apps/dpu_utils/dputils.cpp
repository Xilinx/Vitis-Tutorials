...
{
    DPUTensor *conv_in_tensor = dpuGetInputTensor(task, nodeName);

    // Copy input image to tensor start address
    char *tensorAddr = (char *)dpuGetTensorAddress(conv_in_tensor);
    memcpy(tensorAddr, image.data, dpuGetTensorSize(conv_in_tensor));
