...
void ntt(uint16_t p_in[N], uint16_t p_out[N]) {
    ...
    // it's the first version where #pragma HLS DATAFLOW could be used. 
    ...
    ntt_stage2: for(start = 0; start < N; start = j + len) {
        ...
        ntt_stage2i: for(j = start; j < start + len; j++) {
            #pragma HLS PIPELINE
            int16_t t = fqmul(zeta, p_stage1[j + len]);
            p_stage2[j + len] = p_stage1[j] - t;
            p_stage2[j] = p_stage1[j] + t;
        }
    }
    ...
    ntt_stage4: for(start = 0; start < N; start = j + len) {
        ...
        ntt_stage4i: for(j = start; j < start + len; j++) {
            #pragma HLS PIPELINE
            int16_t t = fqmul(zeta, p_stage3[j + len]);
            p_stage4[j + len] = p_stage3[j] - t;
            p_stage4[j] = p_stage3[j] + t;
        }
    }
    ...
    ntt_stage5: for(start = 0; start < N; start = j + len) {
        ...
        ntt_stage5i: for(j = start; j < start + len; j++) {
            #pragma HLS PIPELINE
            int16_t t = fqmul(zeta, p_stage4[j + len]);
            p_stage5[j + len] = p_stage4[j] - t;
            p_stage5[j] = p_stage4[j] + t;
        }
    }
    ...
    ntt_stage6: for(start = 0; start < N; start = j + len) {
        ...
        ntt_stage6i: for(j = start; j < start + len; j++) {
            #pragma HLS PIPELINE
            int16_t t = fqmul(zeta, p_stage5[j + len]);
            p_stage6[j + len] = p_stage5[j] - t;
            p_stage6[j] = p_stage5[j] + t;
        }
    }
    ...
    ntt_stage7: for(start = 0; start < N; start = j + len) {
        ...
        ntt_stage7i: for(j = start; j < start + len; j++) {
            #pragma HLS PIPELINE
            int16_t t = fqmul(zeta, p_stage6[j + len]);
            p_out[j + len] = p_stage6[j] - t;
            p_out[j] = p_stage6[j] + t;
        }
    }
}
