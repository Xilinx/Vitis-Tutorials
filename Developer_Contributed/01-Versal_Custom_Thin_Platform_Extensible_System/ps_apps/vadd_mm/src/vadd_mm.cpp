...
int main(int argc, char* argv[]) {
  ...
  // Check command line
  if(argc < 2) {
    std::cout << "Usage: " << argv[0] <<" <xclbin> <samples>" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  std::cout << "INFO: samples = " << samples << std::endl;
  std::cout << "INFO: bsize   = " << bsize   << std::endl;
  ...
  std::cout << "PASSED: auto in_01_bo = xrt::bo(my_device, bsize, XCL_BO_FLAGS_NONE, in_01.group_id(0))" << std::endl;

  auto in_0_bo_mapped = in_0_bo.map<TYPE_DATA*>();
  auto in_1_bo_mapped = in_1_bo.map<TYPE_DATA*>();
  std::cout << "PASSED: auto in_01_bo_mapped = = in_01_bo.map<TYPE_DATA*>()" << std::endl;
  ...
  std::cout << "PASSED: in_01_bo.sync(XCL_BO_SYNC_BO_TO_DEVICE)" << std::endl;
  ...
  std::cout << "PASSED: auto out_bo_mapped = out_bo.map<TYPE_DATA*>()" << std::endl;
  ...
  std::cout << std::endl << "INFO: Waiting for kernels to end..." << std::endl << std::endl;

  in_0_run.wait();
  std::cout << "PASSED: in_0_run.wait()" << std::endl;

  in_1_run.wait();
  std::cout << "PASSED: in_1_run.wait()" << std::endl;

  dut_run.wait();
  std::cout << "PASSED: dut_run.wait()" << std::endl;

  out_run.wait();
  std::cout << "PASSED: out_run.wait()" << std::endl;

  // Retrieving the results
  out_bo.sync(XCL_BO_SYNC_BO_FROM_DEVICE);
  std::cout << "PASSED: out_bo.sync(XCL_BO_SYNC_BO_FROM_DEVICE)" << std::endl;
  ...
  for(uint32_t i = 0; i < samples; i++) {
    ...
    if(out_bo_mapped[i] != golden) {
      std::cout << "ERROR: " << i << "; Expected Result: " << golden << "; Kernel Result: " << out_bo_mapped[i] << std::endl;
      ++error_cnt;
    }
  }
