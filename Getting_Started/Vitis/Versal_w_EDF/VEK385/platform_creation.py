...
def generate_application():    
    ...
    status = comp.import_files(from_loc="", files=["../aieml/input.h", "../aieml/host.cpp", "../aieml/golden.h", "./_aie/ps/c_rts/aie_control_xrt.cpp"])
    comp = client.get_component(os.path.join(current_path, "application"))
    status = comp.set_sysroot(sysroot=sysroot)
    comp = client.get_component(os.path.join(current_path, "application"))
    comp.set_app_config(key = 'USER_CMAKE_CXX_STANDARD',values = '17')
    comp.set_app_config(key = 'USER_INCLUDE_DIRECTORIES',values ='../../aieml/')
    comp.build()
