...
template <typename TT_DATA = cint16, size_t BUF_SIZE = 256, size_t MAR_SIZE = 32>
class my_fir16 {
  private:
  ...
  public:
    ...
    my_fir16();
    // Run:
    void __attribute__ ((noinline)) run(const int16 (&coeffs)[16], adf::input_buffer<TT_DATA, adf::extents<BUF_SIZE>, adf::margin<MAR_SIZE>> & __restrict sig_i, adf::output_buffer<TT_DATA, adf::extents<BUF_SIZE>> & __restrict sig_o );

    static void registerKernelClass()
    {
      REGISTER_FUNCTION(my_fir16::run);
...
class my_stream_32b_dm {
  ...
  public:
    ...
    static void registerKernelClass()
    {
      REGISTER_FUNCTION(my_stream_32b_dm::run);
      //register_parameter(coeffs);
    }
};

template <typename TT_DATA = cint16>
class my_stream_32b_opt_dm {
  private:
    typedef TT_DATA MY_TT_DATA;
  ...
  public:
    ...
    // constructor
    my_stream_32b_opt_dm();
    // run:
    void __attribute__ ((noinline)) run(input_stream<MY_TT_DATA> * __restrict sig_i, output_stream<MY_TT_DATA> * __restrict sig_o );

    static void registerKernelClass()
    {
      REGISTER_FUNCTION(my_stream_32b_opt_dm::run);
      //register_parameter(coeffs);
    }
};
