...
% Create graph object:
my_axis1to2 = vfs.hlsKernel(input_files = '../ip/axis1to2/src/axis1to2.cpp', hls_function = "axis1to2");
...
% First stage, copy input data using axis1to2 HLS block
[fir_in0, fir_in1] = my_axis1to2.run(varray.cint16(bb));
% Second stage, run signal through the two FIR filters
[fir_out0, fir_out1] = my_aiefir.run(fir_in0, fir_in1, varray.int16(fliplr(coe_lpass)), varray.int16(fliplr(coe_hpass)));
% Third stage, add the two filter results
vadd_out0 = my_vadd.run(fir_out0, fir_out1);

% Recast vfs types to matlab types
out0 = int16(vadd_out0); 

%% Reference model
y_ref_lp = round((bits-2)*filter(coe_lpass, 2^(n_shift), bb))/(bits-2);
y_ref_hp = round((bits-2)*filter(coe_hpass, 2^(n_shift), bb))/(bits-2);
y_ref = y_ref_lp + y_ref_hp;
...
out0_fd = (20*log10(1/N*abs(fftshift(fft(out0(t_range))))/fft_size*2));
y_ref_fd = (20*log10(1/N*abs(fftshift(fft(y_ref(t_range_trim))))/fft_size*2));


%% Plotting combined VSS system
fig1 = figure(3); clf;
...
title(ax_fd_in, 'Input Spectrum', [num2str(fft_size) ' point FFT']);

ax_td_out=nexttile; hold on;
plot(t_range, real(out0(t_range)), 'b');
plot(t_range, real(y_ref(t_range_trim)), 'ko');
...
plot(fd_axis, out0_fd(fft_size/2:fft_size), 'b');
plot(fd_axis, y_ref_fd(fft_size/2:fft_size), 'ko');
title(ax_fd_out, 'AIE+PL out0 Spectrum', [num2str(fft_size) ' point FFT']);
legend({'out0', 'y\_ref\_fd'});
ylabel('Magnitude [dB]');
xlabel('Frequency [MHz]');

ax_td_err=nexttile; hold on;
plot(t_range, real(out0(t_range)) - int16(real(y_ref(t_range_trim))'), 'R');
...
legend({'y\_fd', 'out\_fd', 'y\_ref\_fd'});
ylabel('Magnitude [dB]');
xlabel('Frequency [MHz]');
