...
for i=1:4
    assert(all(double(in_data_cell{i})*i == output_data{i}))
end
...
% check to see if output is correct
assert(all(double(input_data(:,1))*1 == output_data1))
assert(all(double(input_data(:,2))*2 == output_data2))
assert(all(double(input_data(:,3))*3 == output_data3))
assert(all(double(input_data(:,4))*4 == output_data4))
...
% check to see if output is correct
for i=1:4
    assert(all(double(input_data(:,i))*i == output_data{i}))
end
