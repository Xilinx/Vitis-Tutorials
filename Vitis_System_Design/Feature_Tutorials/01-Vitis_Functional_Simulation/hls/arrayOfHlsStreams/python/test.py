...
for i in range(4):
    assert(np.all(np.asarray(input_data[i])*(i+1) == np.asarray(output_data[i])))
...
assert(np.all(np.asarray(input_data[0])*1 == np.asarray(out[0])))
assert(np.all(np.asarray(input_data[1])*2 == np.asarray(out[1])))
assert(np.all(np.asarray(input_data[2])*3 == np.asarray(out[2])))
assert(np.all(np.asarray(input_data[3])*4 == np.asarray(out[3])))
