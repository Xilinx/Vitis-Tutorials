...
RGBPixel *tmpout = outFrame.data();
// std::cout << "base address of outFrame "<<outFrame.data();
std::cout << "frame bytes " << frame_bytes <<std::endl;    
for(int i=0; i< 5120*4*4; i++)
    {
    // std::cout<<"output b "<< (int)tmpout->b << "output g " <<(int)tmpout->g << "output r " <<(int)tmpout->r<<std::endl;
 //   tmpout++;
    }
//std::cout <<"size of the output buffer " << outFrame.size() <<std::endl;
 //std::cout << "base address of outFrame after teh print out "<<outFrame.data();
       ...
       //     RGBPixel *a = outFrame.data();
       //    size_t bytes_written_a = fwrite(a, 1, frame_bytes, streamOut);
