...
public:
    ...
    core[(yoff&1)*(len-1)]         = kernel::create(bf8x8_fst_api);
    source(core[(yoff&1)*(len-1)]) = "bf8x8_fst_api.cpp" ;

    // Middle kernels
    for(unsigned i=1;i<len-1;i++){
        core[i] = kernel::create(bf8x8_mid_api);
        source(core[i])  = "bf8x8_mid_api.cpp" ;
    }
		
    // Last kernel in the chain
    core[(len-1)*(1-(yoff&1))] = kernel::create(bf8x8_lst_api);
    source(core[(len-1)*(1-(yoff&1))])  = "bf8x8_lst_api.cpp" ;
...
public:
    ...
    DL64A32L(){
        ...
        for(unsigned k=0;k<4;k++){
            ...
            connect<>(din[k],bf4.din[k]);
            connect<>(din[k],bf5.din[k]);
            connect<>(din[k],bf6.din[k]);
            connect<>(din[k],bf7.din[k]);			
        }
        ...
        // connect output
        connect<>(bf0.out,out[0]);
        connect<>(bf1.out,out[1]);
        connect<>(bf2.out,out[2]);
        connect<>(bf3.out,out[3]);
        connect<>(bf4.out,out[4]);
        connect<>(bf5.out,out[5]);
        connect<>(bf6.out,out[6]);
        connect<>(bf7.out,out[7]);	
    }; 
