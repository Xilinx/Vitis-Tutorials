...
This example combines first run with `heat_map`, `conflicts`, and `input_bandwidths` metrics and second run with `execution`, `dma_locks` and `output_bandwidth` metrics. Use step 1.5 to open one profiling run data and click on `+` from GUI, highlighted in red square to combine second profiling run data.
...
#### AI Engine tile profiling data
1. `Active Time (ms)` = `Stall Time (ms)` + `Active Utilization Time (ms)`.
2. 0.061 milliseconds is spent on `Vector Instruction Utilization time (ms)` during 0.069 milliseconds `Active Utilization Time (ms)`. 
...
1. `Memory Conflict Time (ms)` indicates memory access conflicts time uring AI Engine execution. Recommend to rerun `aiesimulator` with `-enable-memory-check` option to check design memory access conflicts.
