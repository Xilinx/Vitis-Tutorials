...
To use the tool, binary container, for example, xclbin must be loaded first after board boots up then issue commands to obtain information from, or, configure the kernels.
...
Step 2: Get detailed status information for the device. Where `TYPE` can be `aie`, `aieshim`, `all`, `debug-ip-status`, `dynamic-regions`, `error`, `host`, `memory`, `pcie-info`, or `platform`.
```bash
...
**Note**: For the `xbutil examine -d 0000:00:00.0 -r aie` command:
...
Step 3: Burn the generated `sd_card.img` to the SD card, and insert the SD card into SD card slot of the VCK190 board and power up.
...
Issue command `xbutil2 examine --report error` and/or with formatted output `xbutil2 examine --report error --format JSON-2020.2` to VCK190 console to get output from console.
