...
* There are (64x64x7) bfloat16 weights that need to be stored, which have a total storage requirement of 56KB (assuming single_buffer). The weights are initialized from the host at run-time during initialization.
