...
|  ---  |  ---  |
|-v | Verbose output. |
|--pl-freq=400| Set the PLIO interface to the AI Engine to 400 MHz. All the AXI4-Stream interfaces from the PL kernels to the AI Engine will operate at this frequency. |
...
|-workdir=Work| Specify the Work directory name.|
...
The fundamental beamforming kernel computes a matrix multiplication on a (8x8) coefficient matrix times a (8x1) input data matrix which results in a (8x1) output data matrix. The beamforming kernel repeats the matrix multiplication computation on as many (8x1) input data matrices as are fed into it.
...
Review the `test_bf_3x.cpp` AI Engine application. Note that all the input and output PLIO ports for the `TEST_BF` graph are instantiated. Then we instantiate the virtual platform and connect the virtual platform's `src` and `sink` arrays to PLIOs.
...
* XAPP1352 Beamforming Implementation on AI Engine (TODO: Add link) 
