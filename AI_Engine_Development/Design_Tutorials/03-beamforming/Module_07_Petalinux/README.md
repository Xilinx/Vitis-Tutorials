...
To create, configure, and build the PetaLinux project with a single command, run the following:
...
Creation of the PetaLinux software platform can be broken down into three phases: create, config, and build. The `make petalinux` step performs all these steps with one command.
...
Next, edit the `build/vck190_linux/project-spec/configs/rootfs_config` file. This is the configuration file used for the root file system. By default, most config options are set to "is not set". You need to update the following config options to ``=y``:  
...
Next, create the ``vck190_linux/project-spec/meta-user/recipes-kernel/linux/linux-xlnx/bsp.cfg`` file. This is a U-Boot fragment config file with the following config options:
...
```
This generates ``BOOT.BIN``, ``BOOT_bh.bin``, and ``qemu_boot.img`` in the ``build/vck190_petalinux/images/linux`` directory. The default DTB load address is 0x1000. For more information, see the Bootgen User Guide ([UG1283](https://www.xilinx.com/search/support-keyword-search.html#q=ug1283)).
