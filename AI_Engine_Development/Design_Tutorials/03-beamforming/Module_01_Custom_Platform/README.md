...
|build/hw/pfm_decls.tcl|Defines clock IDs to the output clock generated by the clocking wizard.|
...
|build/vck190_v1_0_pfm.tcl|Create a new platform by importing an XSA. Creates and configures an aiengine, xrt, and standalone domain in the platform.|
...
|build/sw/src/boot/|Directory which has all the files listed in file names provided as part of the qemu-args and pmuqemu-args options. It is the directory to generate components after the Linux Image build.|
|build/sw/src/a72/xrt/image|For domains with Linux as the OS, use pre-built Linux images from this directory when creating the PetaLinux project. This option is valid only for Linux domains.|
...
|Filename|Description|
|  ---  |  ---  |
|build/hw/build/NOC_Power.xpe| Power estimates generated from the Vivado command.|
...
|build/sw/build/vck190_v1_0|Generated from the XSCT command. Contains the final platform file vck190_v1_0.xpfm used by the Vitis software platform later on.|
...
The software platforms and final XPFM platform file are generated by the ``xsct`` command:
...
Next, connect the NoC up to other parts of the design. First, the `ddr4_dimm1` port created at the beginning of the `dr.bd.tcl` script is connected to the NoC `CH0_DDR4_0` interface. This configures the NoC to have a single memory controller port.
...
Next, the `vck_190_v1_0_xsa.tcl` script sets up the platform for hardware emulation. When creating emulation-capable platforms, CIPS and NoC must use TLM (transaction-level modeling) as the simulation model.  
...
After creating the hardware platform (XSA) with the AI Engine IP through Vivado, import this platform into the Vitis environment and create the software platform. Add the aie_runtime, Linux, and standalone (bare-metal) domains into the platform. In this section you will review important aspects of the `vck_190_v1_0_pfm.tcl` script used to generate the Vitis project. Open this script, review the comments, follow the notes in the next sections of this module.
...
*  Vivado Design Suite Tcl Command Reference Guide ([UG835](https://www.xilinx.com/support/documentation/sw_manuals/xilinx2020_2/ug835-vivado-tcl-commands.pdf))
