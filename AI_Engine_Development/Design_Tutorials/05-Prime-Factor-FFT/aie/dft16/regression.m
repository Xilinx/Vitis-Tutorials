...
function regression(x86,settings)
   if     (nargin == 0) x86 = 1; settings = [2,8*63,16];
   elseif (nargin == 1)          settings = [2,8*63,16];
   ...
   % Alternate every four samples on ss0 then every four samples on ss1
   act_o = zeros(Nport,4,Ntransform*Nfft/Nport/4);
   gld_o = zeros(Nport,4,Ntransform*Nfft/Nport/4);
   for pp = 1 : Nport
