...
#include "pfa1008_transpose2.h"

using namespace std;
using namespace pfa1008_transpose2;

// Approach:
// 1) Collect & unpack 9x4=36 samples each from ss0 and ss1 (4 transforms on each)
// 2) Write back to linear order using strided 7-step address pattern @ 8 samples per clock
// 2) Read from buffer using TBD strided address @ 8 samples per clock
// 3) Transmit alternating 4 samples across ss0 and ss1
...
#pragma HLS array_partition variable=save1 dim=1
  ...
  // Unpack samples from input streams (incoming alternating 9-point transforms on ss0 then ss1):
...
#pragma HLS array_partition variable=inss1 dim=1
  ...
  // cout << "stream inputs: ss0: "
  //      << inss0[0] << " " << inss0[1] << " " << inss0[2] << " " << inss0[3] << " ss1: "
  //      << inss1[0] << " " << inss1[1] << " " << inss1[2] << " " << inss1[3] << endl;
...
#pragma HLS reset variable=running
  ...
  static ap_uint<3> rd_cnt7  = 0;
  static ap_uint<4> rd_cnt9  = 0;
  static ap_uint<4> rd_cnt16 = 0;
#pragma HLS reset variable=rd_addr
#pragma HLS reset variable=rd_cnt7
#pragma HLS reset variable=rd_cnt9
#pragma HLS reset variable=rd_cnt16

  static TT_ADDR wr_addr = 0;
  static TT_ADDR wr_start = 1;
#pragma HLS reset variable=wr_addr
#pragma HLS reset variable=wr_start
...
#pragma HLS dependence variable=buff type=intra false
  ...
  // cout << "cnt7: " << rd_cnt7 << " cnt9: " << rd_cnt9 << " cnt16: " << rd_cnt16 << " " << "rd_addr: ";

  // Write 8 samples per PL cycle using 7-strided addressing
...
#pragma HLS array_partition variable=read_data dim=1
 ...
 READ_WRITE : for (int ss=0,offW=0,offR=0; ss < 8; ss++,offW+=7,offR+=63) {
    ...
    // cout << rd_addr_use.range(1,0) << " ";
    // cout << wr_addr_use.range(1,0) << " ";
  }
  // cout << endl;
  ...
  if ( running == 1 ) {
    // Update read address:
    if ( rd_cnt16 == ap_uint<4>(8) ) { // Check last iteration of inner loop
      if ( rd_cnt9 == ap_uint<4>(8) ) { // Check last iteration of middle loop
        if ( rd_cnt7 == ap_uint<3>(6) ) { // Check last iteration of outer loop
          ping = !ping;
          rd_addr = 0;
          rd_cnt7 = 0;
          rd_cnt9 = 0;
          ...
          rd_cnt9 = 0;
          rd_cnt16 = 0;
          rd_addr = TT_ADDR(rd_cnt7);
        }
      }
      else { // Middle loop body
        rd_cnt16 = 0;
        rd_cnt9++;
        rd_addr = TT_ADDR(7*rd_cnt9 + rd_cnt7);
      }
    }
    else { // Inner loop body
      rd_cnt16 += 8;
      rd_addr = TT_ADDR(rd_addr + 8*63);
    }

    // Update write address:
    bool last_wr = ( wr_addr >= TT_ADDR(NFFT-8*7) );
    if ( last_wr == 1 ) {
      if ( wr_start == TT_ADDR(7) ) {
        wr_start = 1;
        wr_addr = 0;
      }
      else {
      ...
      wr_addr += TT_ADDR(8*7);  // 8 hops of Stride-7
    }
  } // running
  running = 1;
}
...
{
  // Need to write 4 samples alternately to ss0 and ss1:
  static bool        running = 0;
...
#pragma HLS reset variable=startup
  ...
  TT_AXI4 ss0_data = ( trans[3], trans[2], trans[1], trans[0] );
  TT_AXI4 ss1_data = ( trans[7], trans[6], trans[5], trans[4] );
...
void pfa1008_transpose2_wrapper( TT_STREAM sig_i[SSR_IO], TT_STREAM sig_o[SSR_IO] )
{
