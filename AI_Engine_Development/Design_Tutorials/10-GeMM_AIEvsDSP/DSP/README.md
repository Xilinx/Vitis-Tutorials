<table class="sphinxhide" width="100%">
 <tr width="100%">
    <td align="center"><img src="https://raw.githubusercontent.com/Xilinx/Image-Collateral/main/xilinx-logo.png" width="30%"/><h1>2022.2 Versal Matrix Multiplication using DSP58 Tutorial</h1>
   </td>
 </tr>
</table>
...
See [this page](https://www.xilinx.com/html_docs/xilinx2022_2/vitis_doc/vitiscommandcompiler.html#wrj1504034328013) for a detailed description of all Vitis compiler switches. The following table provides a summary of the switches used. 
...
See [this page](https://www.xilinx.com/html_docs/xilinx2022_2/vitis_doc/buildingdevicebinary.html#mjs1528399150499) for a detailed description of Vitis linking options. The following table provides a summary of the switches used. 
...
|--profile.trace_memory \<FIFO\>:\<size\>\|\<MEMORY\>[\<n\>]|When building the hardware target \(-t=hw\), use this option to specify the type and amount of memory to use for capturing trace data. See [this page](https://www.xilinx.com/html_docs/xilinx2022_2/vitis_doc/vitiscommandcompiler.html#lpy1600804966354__section_bmy_v3z_54b) for detailed profiling options.|
...
See [this page](https://xilinx.github.io/XRT/2022.2/html/index.html) for XRT documentation. See [this page](https://docs.xilinx.com/r/en-US/ug1393-vitis-application-acceleration/Host-Programming4) for details of host application programming.
...
32x32 Matrix-Matrix Multiplication is decomposed into Matrix-Vector multiplication. Matrix B column vector is multiplied by each Row of Matrix A.  

Matrix B Column vector is cascaded to B port. Elements in one row of Matrix A are cascaded to Port A of DSPs in one cascade chain 

It takes 32 DSP operating clocks to generate product of one Matrix A row and one Matrix B column. Then we move to next column of Matrix B, and this column is also multiplied with all 32 Rows of Matrix A, which takes another
32 clocks. After 32 column vectors, 32x32 matrix multiplication is completed.

Below example shows how 1Kx1K matrix multiplication is done using 32x32 sub-matrix multiplication

1Kx1Kx1K Matrices are represented as follows, where each box is 32x32 matrix –
...
Matrix A00 is read first, and it is multiplied with Matrix B00, B10, B20, … B31,0 (First submatrix column of matrix B)
Matrix A00 is first multiplied with Matrix B00, which is the basic 32x32 matrix multiplication. Excluding initial latency, this operation completes in 
16 clocks, (because 2 rows/columns of subnatrix are read on one clock cycle) (it equals 32 DSP operating clocks) and generates 1K partial Sums. These partial sums are written to 64 partial Sum BRAMs (16 locations written to 32 partial Sum BRAMs. Partial Sum BRAM config is 1Kx64)
After completing A00xB00 multiplication, Column logic wraps around to Column 0 of Matrix A00, while Row logic moves ahead with submatrix B10. It takes
another 16 clocks, to generate 1K more partial Sums which are also written to Partial Sum BRAMs.
In the last, i.e 32nd iteration for Column A00, A00 is multiplied with B31,0 to generate 1K more partial Sums, and that completes A00 multiplication. This operation takes total 1K clocks.

Next we will read A10 of Matrix A and it is multiplied with 2nd submatrix column of Matrix B (B01, B11, …B31,1). Each submatrix multiplication
will generate 1K partial Sums which are added to the partial sums already stored in the BRAM, and the result is stored back to partial sum BRAM. This operation also takes another 1K clocks
Thus we will keep moving along the first column of Matrix A, and multiply each column submatrix with next column sub-matrices of Matrix B to generate further partial sums for first column of final output
After 32 iterations, first column of the final output matrix is generated, and this data is written to Output BRAM instead of partial Sum BRAM. This operation will be completed in 32K clocks

Then we will move to the next column of Matrix A and repeat the steps
After 32 such iterations, 1kx1Kx1K matrix multiplication will be completed. It will take a total of 512K clocks for 1Kx1K matrix multiplication to complete 
...
GeMM DSP RTL design can be divided into 2 main parts -
 ...
 Second part involves data mover logic for writing Matrix A and B data and to read the matrix output from host application. Module gemm_large_ocm is the top level module which instantiates both data mover and the core logic, and that is the top level PL kernel module
...
Modules instantiated under gemm_large_ocm are as follows -

1. ps_slave - This is AXI Lite slave to write and read URAMs and control registers from host app
2. gemm_top 
...
Resource Utilization in this tutorial is same for 32x32x32 matrix is as follows -
...
Resource Utilization in this tutorial is same for rest of the matrix sizes, and is as follows -
...
|                    | Per matrix (int16) |   no of clocks  |                  |               | 
+--------------------+--------------------+-----------------+------------------+---------------+
|        32x32x32    |     1024           |        34       |     0.091        |  1.10 x 10^7  |
|        64x64x64    |     4096           |       130       |     0.346        |  2.89 x 10^6  |
|     128x128x128    |    16384           |      1026       |     2.736        |  3.65 x 10^5  |
|     256x256x256    |    65536           |      8194       |    21.850        |  4.57 x 10^4  |
|     512x512x512    |   262144           |     65538       |   174.768        |  5.72 x 10^3  |
|  1024x1024x1024    |  1048576           |    524290       |  1398.106        |  7.15 x 10^2  |
+--------------------+--------------------+-----------------+------------------+---------------+
```
</details>
...
Since TOPS and power utilization for DSP based martix multiplication is more or less independent of martix size. TOPS is 1.49 and TOPs per Watt is 0.28 to 0.31 in this design 
...
+--------------------+-------------------+------------------+---------------+---------+--------------------+-------------+-------------------------+--------------+------------------+--------------------+----------------+
|        32x32x32    |     12000         |    0.091         | 1.10 x 10^7   | 1.407   |        NA          |    NA       |         NA              |    NA        |      NA          |      4929.8        |    0.285       | 
|        64x64x64    |     12000         |    0.346         | 2.89 x 10^6   | 1.479   |        NA          |    NA       |         NA              |    NA        |      NA          |      4155.1        |    0.355       |
|     128x128x128    |      6000         |    2.736         | 3.65 x 10^5   | 1.497   |        NA          |    NA       |         NA              |    NA        |      NA          |      4742.0        |    0.315       |
|     256x256x256    |      3000         |   21.850         | 4.57 x 10^4   | 1.499   |        NA          |    NA       |         NA              |    NA        |      NA          |      5201.4        |    0.288       |
|     512x512x512    |      1500         |  174.768         | 5.72 x 10^3   | 1.499   |        NA          |    NA       |         NA              |    NA        |      NA          |      5035.9        |    0.297       |
|  1024x1024x1024    |       750         | 1398.106         | 7.15 x 10^2   | 1.499   |        NA          |    NA       |         NA              |    NA        |      NA          |      4765.4        |    0.314       |
+--------------------+-------------------+------------------+---------------+---------+--------------------+-------------+-------------------------+--------------+------------------+--------------------+----------------+
```
