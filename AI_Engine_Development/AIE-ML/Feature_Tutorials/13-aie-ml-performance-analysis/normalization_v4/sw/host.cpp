...
int run_plio_graph(const xrt::device &device,const xrt::uuid &id,int iter_graph){
	...
	int OUTPUT_SIZE=output_size_in_bytes/16/3; //128 bits interface, 3 PL kernels
	...
	auto mm2s2_run = mm2s2(nullptr, OUTPUT_SIZE);
	auto mm2s3_run = mm2s3(nullptr, OUTPUT_SIZE);
	Timer timer;
