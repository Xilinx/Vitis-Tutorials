...
% Extract matrix using a 4x2 tiling pattern
% major = row (0) or column (1)

function [vec] = tiling_4x2( mat_i, major )
   [R,C] = size(mat_i);
   assert(mod(R,4)==0,'# of rows must be multiple of 4');
   assert(mod(C,2)==0,'# of cols must be multiple of 2');
   nR = R/4;
   nC = C/2;
