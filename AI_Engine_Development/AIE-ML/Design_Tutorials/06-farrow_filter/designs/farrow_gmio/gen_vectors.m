...
% Input samples
% samples - complex, so multiple by two for real and imag parts
NO_OF_SAMPLES = Nsamp * 2;
fid0_i = fopen('data/input_sig_i.h','w');
fprintf(fid0_i,'short int input_sig_i[%d] = {\n',NO_OF_SAMPLES);
fid1_i = fopen('data/input_del_i.txt','w');
last = numel(sig_i) - 1;
...
fid2_i = fopen('data/input_del_i_optimized.h','w');
fprintf(fid2_i,'int input_del_i[%d] = {\n',Nsamp);
last = numel(del_i) - 8 + 1;
...
% Output samples
fid_o = fopen('data/output_sig_o.h','w');
fprintf(fid_o,'short int output_sig_o[%d] = {\n',NO_OF_SAMPLES);
last = numel(sig_o) - 1;
