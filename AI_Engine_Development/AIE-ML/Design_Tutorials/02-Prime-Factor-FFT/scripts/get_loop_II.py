            ...
            if match:
                ...
                with open(fname) as file:
                    for line in file:
                        ...
                        if match:
                            min_II = match.group(1)
#                        match = re.search('(resume algo).*after folding: (\d+).*folded over (\d+) iterations',line)
                        match = re.search('.*after folding: (\d+).*folded over (\d+) iterations',line)
                        if match:
#                            print("*** [LOOP_II] *** Tile " + tileid + " minII = " + min_II + " achieves II = " + match.group(2) + " (folded over " + match.group(3) + " iterations)")
                            print("*** [LOOP_II] *** Tile " + tileid + " minII = " + min_II + " achieves II = " + match.group(1))
#                            break
