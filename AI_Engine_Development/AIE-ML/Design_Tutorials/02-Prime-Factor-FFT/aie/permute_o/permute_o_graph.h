...
class permute_o_graph : public graph {
public:
  ...
  #include "permute_o_bd.h"

  // Constructor:
  permute_o_graph( void )
  {
    // Create buffer:
    MT_buff = shared_buffer<TT_DATA>::create({NFFT7,NFFT9,NFFT16},1,1);
    write_access(MT_buff.in[0]) = tiling(write_p0);
    read_access(MT_buff.out[0]) = tiling(read_p0);
