...
dft9_1::dft9_1( TT_TWID (&coeff_i)[COEFF_DEPTH] ) : coeff(coeff_i)
{
...
void dft9_1::run( input_cascade<TT_ACC>* __restrict acc_i,
                  ...
                  output_cascade<TT_ACC>* __restrict acc_o )
{
  ...
  {
    compute0 = readincr_v<8>( acc_i );
    compute1 = readincr_v<8>( acc_i );
    compute2 = readincr_v<8>( acc_i );
    compute3 = readincr_v<8>( acc_i );
    compute4 = readincr_v<8>( acc_i );
    compute5 = readincr_v<8>( acc_i );
    compute6 = readincr_v<8>( acc_i );
    compute7 = readincr_v<8>( acc_i );
    ...
    compute0.mac(                                   data0.extract<4>(1), vc0 );
    compute1.mac(                                   data0.extract<4>(1), vc1 );
    compute2.mac( (aie::shuffle_down     (data0,1      )).extract<4>(3), vc0 );
    compute3.mac( (aie::shuffle_down     (data0,1      )).extract<4>(3), vc1 );
    compute4.mac( (aie::shuffle_down     (data0,2      )).extract<4>(5), vc0 );
    compute5.mac( (aie::shuffle_down     (data0,2      )).extract<4>(5), vc1 );
    compute6.mac( (aie::shuffle_down_fill(data0,data1,3)).extract<4>(7), vc0 );
    compute7.mac( (aie::shuffle_down_fill(data0,data1,3)).extract<4>(7), vc1 );
    ...
    compute0.mac(                        data1.extract<4>(2), vc0 );
    compute1.mac(                        data1.extract<4>(2), vc1 );
    compute2.mac( (aie::shuffle_down(data1,1)).extract<4>(4), vc0 );
    compute3.mac( (aie::shuffle_down(data1,1)).extract<4>(4), vc1 );
    compute4.mac( (aie::shuffle_down(data1,2)).extract<4>(6), vc0 );
    compute5.mac( (aie::shuffle_down(data1,2)).extract<4>(6), vc1 );
    compute6.mac( (aie::shuffle_down(data2,3)).extract<4>(2), vc0 );
    compute7.mac( (aie::shuffle_down(data2,3)).extract<4>(2), vc1 );
