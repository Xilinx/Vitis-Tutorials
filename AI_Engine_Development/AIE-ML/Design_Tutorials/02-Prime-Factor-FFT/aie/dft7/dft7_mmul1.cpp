...
dft7_1::dft7_1( TT_TWID (&coeff_i)[COEFF_DEPTH] ) : coeff(coeff_i)
{
...
void dft7_1::run( input_cascade<TT_ACC>* __restrict acc_i,
                  ...
                  output_cascade<TT_ACC>* __restrict acc_o )
{
    ...
    chess_prepare_for_pipelining
  {
    ...
    compute1.mac( aie::shuffle_up(data0,1).extract<4>(3), vc0 );
    compute2.mac( aie::shuffle_up(data0,2).extract<4>(5), vc0 );
    compute3.mac( aie::shuffle_up(data0,3).extract<4>(7), vc0 );
    compute4.mac(                    data1.extract<4>(0), vc0 );
    compute5.mac( aie::shuffle_up(data1,1).extract<4>(2), vc0 );
    compute6.mac( aie::shuffle_up(data1,2).extract<4>(4), vc0 );
    compute7.mac( aie::shuffle_up(data1,3).extract<4>(6), vc0 );
