...
#define N_KERS N_INST/REPS              // Number of kernels
        ...
        // Creating the shared buffers and connecting them to the I/Os and kernels
            for(int i=0; i<N_IO; i++){
                ...
                write_access(in_mem[i].in[0]) =
                    tiling(
                        {
                            .buffer_dimension = {PLIO_WIDTH, IO_ILV, POINTS},      // Full buffer dimensions
                            .tiling_dimension = {PLIO_WIDTH, IO_ILV, POINTS},      // Tiling for a single PLIO
                            .offset           = {0, 0, 0},
                            .tile_traversal   = {
                                {.dimension=0, .stride=PLIO_WIDTH, .wrap=1},    // Reading one batch at a time,
                                {.dimension=1, .stride=IO_ILV, .wrap=1},           // proceeding with IO_ILV, and
                                {.dimension=2, .stride=POINTS, .wrap=1}         // row by row
                            } 
                        });
                ...
                for(int k=0; k<IO_ILV; k++)
                    ...
                    // Kernel inputs management
                        read_access(in_mem[i].out[cur]) =
                            tiling(                                                                              
                                {
                                    .buffer_dimension = {PLIO_WIDTH, IO_ILV, POINTS},      // Full buffer dimension
                                    .tiling_dimension = {1, 1, POINTS},              // 2D Tiling for each kernel
                                    .offset           = {REPS*j, k, 0},                 // Justifying the read pointer
                                    .tile_traversal   = {
                                        {.dimension=2, .stride=POINTS, .wrap=1},        // Reading all the points of
                        ...
                        connect(k_kernel[int(i*(PLIO_WIDTH*IO_ILV/REPS)+cur)].out[0], out_mem[i].in[cur]);
