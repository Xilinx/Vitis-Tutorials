...
N = 4096                # no. of samples for impulse response
show_plots = true       # display plots?
write_cmatrix = true    # write C matrix to files?
split_eo_coeff = true   # split coefficients into even and odd files
...
if write_cmatrix
    ...
    **********************************************************************/
    ...
    if split_eo_coeff   # allow loading TWO 8-element vectors in one cycle

        # write it out column-wise
        # split it into even and odd columns
        for u = 1 : num_sections

            fname_e = string("C", string(u), "_e.h")    # "even" columns file
            fname_o = string("C", string(u), "_o.h")    # "odd" columns file

            def_header_name_e = string("__C", string(u), "_E_H__")
            def_header_name_o = string("__C", string(u), "_O_H__")

            fid_e = open(fname_e, "w")
            fid_o = open(fname_o, "w")

            @printf(fid_e, "%s\n", copyright_str)
            @printf(fid_e, "#ifndef %s\n", def_header_name_e)
            @printf(fid_e, "  #define %s\n\n", def_header_name_e)
            @printf(fid_e, "  // SIMD matrix of coefficients for IIR filter stage %d (even columns)\n", u)
            @printf(fid_e, "  alignas(16) const float C%d_e[%d] = {\n\n", u, ((12/2) * 8))

            @printf(fid_o, "%s\n", copyright_str)
            @printf(fid_o, "#ifndef %s\n", def_header_name_o)
            @printf(fid_o, "  #define %s\n\n", def_header_name_o)
            @printf(fid_o, "  // SIMD matrix of coefficients for IIR filter stage %d (odd columns)\n", u)
            @printf(fid_o, "  alignas(16) const float C%d_o[%d] = {\n\n", u, ((12/2) * 8))

            for v = 1 : 12
                fid = ((v - 1) % 2 == 0) ? fid_e : fid_o
                for w = 1 : 8
            ...
            @printf(fid_e, "  };\n\n")
            @printf(fid_e, "#endif // %s\n", def_header_name_e)

            @printf(fid_o, "  };\n\n")
            @printf(fid_o, "#endif // %s\n", def_header_name_o)
        ...
        # write it out column-wise
        for u = 1 : num_sections
            fname = string("C", string(u), ".h")
            def_header_name = string("__C", string(u), "_H__")
            fid = open(fname, "w")
            @printf(fid, "%s\n", copyright_str)
            @printf(fid, "#ifndef %s\n", def_header_name)
            @printf(fid, "  #define %s\n\n", def_header_name)
            @printf(fid, "  // SIMD matrix of coefficients for IIR filter stage %d\n", u)
            @printf(fid, "  alignas(16) const float C%d[%d] = {\n\n", u, (12 * 8))
            for v = 1 : 12
                for w = 1 : 8
                    @printf(fid, "    %+1.23e", C[w, v, u])
                    if !((w == 8) && (v == 12))
