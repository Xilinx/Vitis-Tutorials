...
In the Graph view of Vitis Analyzer, you can visualize the stalled path in the graph, which gives an indication of where the stall has happened in the design. By understanding the design behavior, it is also possible to estimate the cause of the hang.
...
Analysis of the VCD file in Vitis Analyzer is similar to analysis in AI Engine simulator. A workaround is to move the VCD file generated by the hardware emulation flow to the working directory and open it:
...
- **Using `devmem` to probe AI Engine registers to see AI Engine status:** By using the `devmem` command, you can read AI Engine registers to see the AI Engine internal status. The register reference can be found in the _Versal adaptive SoC AI Engine Register Reference_ ([AM015](https://docs.amd.com/r/u6ahxKvZt_LkGE2eA77Oeg/root)).
