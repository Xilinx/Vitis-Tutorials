...
int main(int argc, char* argv[])
{
	...
	auto s2mm_2_khdl = xrt::kernel(device, xclbin_uuid, "s2mm:{s2mm_2}");  
	//s2mm and mm2s here, are kernel function names
	
	auto in_bohdl = xrt::bo(device, sizeIn * sizeof(int), mm2s_khdl.group_id(0));
	auto in_bomapped = in_bohdl.map<int32_t*>();
	
	//Host only takes 32-bit input. so (7-iterations * 128 samples * 4bytes(int32)) bytes of memory is required.
	//Feed 7*128 samples 
	memcpy(in_bomapped, Input_x, sizeIn * sizeof(int)); 
