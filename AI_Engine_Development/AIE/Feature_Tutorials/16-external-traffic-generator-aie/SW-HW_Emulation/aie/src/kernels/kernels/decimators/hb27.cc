...
(
      input_buffer<cint16,adf::extents<adf::inherited_extent>,adf::margin<DECIMATOR27_COEFFICIENTS>>  & __restrict inputw,
	output_buffer<cint16> & __restrict outputw
) {
    const int shift = 1 ;
    ...
    auto * symw = &temp_w;
    *symw = *inputw;
    auto SymIter = aie::begin_vector<8>(symw);
    auto InIter = aie::begin_vector<8>(inputw);
    auto OutIter = aie::begin_vector<8> (outputw);
        ...
        chess_prepare_for_pipelining
    {
        ...
        vdata=*InIter++;
        lbuff = upd_w0(lbuff, vdata); (*InIter-=1);
        vdata=*SymIter++;
        ...
        vdata=*InIter++;
        lbuff = upd_w1(lbuff, vdata); (*InIter+=2);
