...
const int ELEM=16;
template<int NUM>
void vect_add(input_buffer<int32,extents<NUM>>& __restrict in,output_buffer<int32,extents<NUM>>& __restrict out,const int32 (&value)[ELEM]){
	auto inIter=aie::begin_vector<ELEM>(in);
	auto outIter=aie::begin_vector<ELEM>(out);
	aie::vector<int32,ELEM> vv=aie::load_v<16>(value);
