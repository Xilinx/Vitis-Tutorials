...
public:
  ...
  static constexpr int  REPEAT                = 128;
  ...
  static constexpr int  TP_API                = 1;
  static constexpr int  TP_PARALLEL_POWER     = 1;
  static constexpr int  Nports = (TP_API == 1 ) ? (2 << TP_PARALLEL_POWER) : 1;
  ...
  fft32_dsplib_graph(void)
  {
    ...
    for (int lane = 0; lane < 2; lane++) {
      location<kernel>(dut_fft.m_r2Comb[lane]) = tile(ORIGIN_X + lane * 2, ORIGIN_Y + TP_CASC_LEN + 1);
    }
  }
};
