...
if (0)
  figure;
  subplot(2,1,1); plot(real(reshape(check_o,1,[])),'b.-'); hold on; plot(real(reshape(all_o,1,[])),'r.--'); hold off;
  subplot(2,1,2); plot(real(reshape(check_o,1,[])-reshape(all_o,1,[])),'k.--'); hold off;
  yline(0.5^15,'r'); yline(-0.5^15,'r');
end
...
write_twiddle_file('twiddle_dft16.h',twidA0,twidA1,twidB0,twidB1,twidC0,twidC1,twidD0,twidD1);
...
%             real(vec(ii+2)).int,imag(vec(ii+2)).int,...
%             real(vec(ii+3)).int,imag(vec(ii+3)).int);
   end
   fclose(fid);
end

function write_twiddle_file(fname,twidA0,twidA1,twidB0,twidB1,twidC0,twidC1,twidD0,twidD1)
   fid = fopen(fname,'w');
