...
public:
  ...
  softmax_graph()
  {
      ...
      smax_krnl = kernel::create_object<softmax_kernel>(std::vector<float>(BUFFSZ), std::vector<float>(BUFFSZ), 
                                                        std::vector<float>(BUFFSZ), std::vector<float>(BUFFSZ));
      ...
      location<parameter>(smax_krnl.param[2]) = location<kernel>(smax_krnl);
      location<parameter>(smax_krnl.param[3]) = location<kernel>(smax_krnl);
  }
};
