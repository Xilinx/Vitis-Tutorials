...
do
   ...
   do
      if [[ ${trgt_lst[$k]} == "hw" ]]
      then
         mk_targets="$base_mk_trgts EN_TRACE=$en_trace TARGET=${trgt_lst[$k]} GEMM_SIZE=${gemm_pt[$i]} PL_FREQ=$freq"
      else
         mk_targets="$base_mk_trgts TARGET=${trgt_lst[$k]} GEMM_SIZE=${gemm_pt[$i]} PL_FREQ=$freq LAUNCH_HW_EMU_EXEC=0"
      fi
      ...
      logName="gemm_${gemm_pt[$i]}x${gemm_pt[$i]}x${gemm_pt[$i]}"_"${trgt_lst[$k]}_dsp"
