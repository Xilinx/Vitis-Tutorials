...
XPAR_VITISREGION_ULBF_DATA_00_BASEADDR,
XPAR_VITISREGION_ULBF_DATA_10_BASEADDR,
XPAR_VITISREGION_ULBF_DATA_20_BASEADDR,
};

uint64_t ulbfCoeffsAddr[NUM_ULBF_COEFFS] = {
XPAR_VITISREGION_ULBF_COEFFS_00_BASEADDR,
XPAR_VITISREGION_ULBF_COEFFS_01_BASEADDR,
XPAR_VITISREGION_ULBF_COEFFS_02_BASEADDR,
XPAR_VITISREGION_ULBF_COEFFS_03_BASEADDR,
XPAR_VITISREGION_ULBF_COEFFS_10_BASEADDR,
XPAR_VITISREGION_ULBF_COEFFS_11_BASEADDR,
XPAR_VITISREGION_ULBF_COEFFS_12_BASEADDR,
XPAR_VITISREGION_ULBF_COEFFS_13_BASEADDR,
XPAR_VITISREGION_ULBF_COEFFS_20_BASEADDR,
XPAR_VITISREGION_ULBF_COEFFS_21_BASEADDR,
XPAR_VITISREGION_ULBF_COEFFS_22_BASEADDR,
XPAR_VITISREGION_ULBF_COEFFS_23_BASEADDR,

};

uint64_t ulbfSlaveAddr[NUM_ULBF_SLAVES] = {
XPAR_VITISREGION_ULBF_O00_BASEADDR,
XPAR_VITISREGION_ULBF_O01_BASEADDR,
XPAR_VITISREGION_ULBF_O02_BASEADDR,
XPAR_VITISREGION_ULBF_O03_BASEADDR,
XPAR_VITISREGION_ULBF_O10_BASEADDR,
XPAR_VITISREGION_ULBF_O11_BASEADDR,
XPAR_VITISREGION_ULBF_O12_BASEADDR,
XPAR_VITISREGION_ULBF_O13_BASEADDR,
XPAR_VITISREGION_ULBF_O20_BASEADDR,
XPAR_VITISREGION_ULBF_O21_BASEADDR,
XPAR_VITISREGION_ULBF_O22_BASEADDR,
XPAR_VITISREGION_ULBF_O23_BASEADDR,
};

void ulbf_data_config_ip(unsigned int i, uint64_t ipBaseAddr, unsigned int niter ) {
	uint64_t csrAddr;
...
void ulbf_data_get_ip_status(unsigned int i, uint64_t ipBaseAddr) {
	xil_printf("---------------------------\n");
	uint64_t addr;
...
void ulbf_coeffs_config_ip(unsigned int i, uint64_t ipBaseAddr, unsigned int niter ) {
	uint64_t csrAddr;
...
void ulbf_coeffs_get_ip_status(unsigned int i, uint64_t ipBaseAddr) {
	xil_printf("---------------------------\n");
	uint64_t addr;
...
void ulbf_slave_config_ip(unsigned int i, uint64_t ipBaseAddr, unsigned int niter ) {
	uint64_t csrAddr = ipBaseAddr + ULBF_SLAVE_CSR_OFFSET ; 
...
void ulbf_slave_get_ip_status(unsigned int i, uint64_t ipBaseAddr) {
	xil_printf("---------------------------\n");
	uint64_t addr;
...
void ulbf_data_check_ram(int ipNum, int * arrPtr0, int * arrPtr1, int * arrPtr2, int * arrPtr3, int * arrPtr4, int * arrPtr5, int * arrPtr6, int * arrPtr7) {
	...
	for (unsigned int i=0; i<5; i++ ) {
		uint64_t addr0   = ulbfDataAddr[ipNum] + ULBF_DATA_RAM0_OFFSET + (i*8);
		uint64_t addr1   = ulbfDataAddr[ipNum] + ULBF_DATA_RAM0_OFFSET + ((i*8)+4);
	...
	for (unsigned int i=0; i<5; i++ ) {
		uint64_t addr0   = ulbfDataAddr[ipNum] + ULBF_DATA_RAM1_OFFSET + (i*8);
		uint64_t addr1   = ulbfDataAddr[ipNum] + ULBF_DATA_RAM1_OFFSET + ((i*8)+4);
		unsigned int data0 = Xil_In32(addr0);
		unsigned int data1 = Xil_In32(addr1);
		int real0 = extractIQ(data0, 0); // 0 for lower 16b and 1 for upper 16b
		int imag0 = extractIQ(data0, 1);
	...
	for (unsigned int i=0; i<5; i++ ) {
		uint64_t addr0   = ulbfDataAddr[ipNum] + ULBF_DATA_RAM2_OFFSET + (i*8);
		uint64_t addr1   = ulbfDataAddr[ipNum] + ULBF_DATA_RAM2_OFFSET + ((i*8)+4);
		unsigned int data0 = Xil_In32(addr0);
		unsigned int data1 = Xil_In32(addr1);
		int real0 = extractIQ(data0, 0); // 0 for lower 16b and 1 for upper 16b
		int imag0 = extractIQ(data0, 1);
	...
	for (unsigned int i=0; i<5; i++ ) {
		uint64_t addr0   = ulbfDataAddr[ipNum] + ULBF_DATA_RAM3_OFFSET + (i*8);
		uint64_t addr1   = ulbfDataAddr[ipNum] + ULBF_DATA_RAM3_OFFSET + ((i*8)+4);
		unsigned int data0 = Xil_In32(addr0);
		unsigned int data1 = Xil_In32(addr1);
		int real0 = extractIQ(data0, 0); // 0 for lower 16b and 1 for upper 16b
		int imag0 = extractIQ(data0, 1);
	...
	for (unsigned int i=0; i<5; i++ ) {
		uint64_t addr0   = ulbfDataAddr[ipNum] + ULBF_DATA_RAM4_OFFSET + (i*8);
		uint64_t addr1   = ulbfDataAddr[ipNum] + ULBF_DATA_RAM4_OFFSET + ((i*8)+4);
		unsigned int data0 = Xil_In32(addr0);
		unsigned int data1 = Xil_In32(addr1);
		int real0 = extractIQ(data0, 0); // 0 for lower 16b and 1 for upper 16b
		int imag0 = extractIQ(data0, 1);
	...
	for (unsigned int i=0; i<5; i++ ) {
		uint64_t addr0   = ulbfDataAddr[ipNum] + ULBF_DATA_RAM5_OFFSET + (i*8);
		uint64_t addr1   = ulbfDataAddr[ipNum] + ULBF_DATA_RAM5_OFFSET + ((i*8)+4);
		unsigned int data0 = Xil_In32(addr0);
		unsigned int data1 = Xil_In32(addr1);
		int real0 = extractIQ(data0, 0); // 0 for lower 16b and 1 for upper 16b
		int imag0 = extractIQ(data0, 1);
	...
	for (unsigned int i=0; i<5; i++ ) {
		uint64_t addr0   = ulbfDataAddr[ipNum] + ULBF_DATA_RAM6_OFFSET + (i*8);
		uint64_t addr1   = ulbfDataAddr[ipNum] + ULBF_DATA_RAM6_OFFSET + ((i*8)+4);
		unsigned int data0 = Xil_In32(addr0);
		unsigned int data1 = Xil_In32(addr1);
		int real0 = extractIQ(data0, 0); // 0 for lower 16b and 1 for upper 16b
		int imag0 = extractIQ(data0, 1);
	...
	for (unsigned int i=0; i<5; i++ ) {
		uint64_t addr0   = ulbfDataAddr[ipNum] + ULBF_DATA_RAM7_OFFSET + (i*8);
		uint64_t addr1   = ulbfDataAddr[ipNum] + ULBF_DATA_RAM7_OFFSET + ((i*8)+4);
		unsigned int data0 = Xil_In32(addr0);
		unsigned int data1 = Xil_In32(addr1);
		int real0 = extractIQ(data0, 0); // 0 for lower 16b and 1 for upper 16b
		int imag0 = extractIQ(data0, 1);
...
void ulbf_coeffs_check_ram(int ipNum, int * arrPtr0, int * arrPtr1, int * arrPtr2, int * arrPtr3, int * arrPtr4, int * arrPtr5, int * arrPtr6, int * arrPtr7) {
	...
	int errCount = 0;
	for (unsigned int i=0; i<5; i++ ) {
		uint64_t addr0   = ulbfCoeffsAddr[ipNum] + ULBF_COEFFS_RAM0_OFFSET + (i*8);
		uint64_t addr1   = ulbfCoeffsAddr[ipNum] + ULBF_COEFFS_RAM0_OFFSET + ((i*8)+4);
		unsigned int data0 = Xil_In32(addr0);
		unsigned int data1 = Xil_In32(addr1);
		int real0 = extractIQ(data0, 0); // 0 for lower 16b and 1 for upper 16b
		int imag0 = extractIQ(data0, 1);
	...
	for (unsigned int i=0; i<5; i++ ) {
		uint64_t addr0   = ulbfCoeffsAddr[ipNum] + ULBF_COEFFS_RAM1_OFFSET + (i*8);
		uint64_t addr1   = ulbfCoeffsAddr[ipNum] + ULBF_COEFFS_RAM1_OFFSET + ((i*8)+4);
		unsigned int data0 = Xil_In32(addr0);
		unsigned int data1 = Xil_In32(addr1);
		int real0 = extractIQ(data0, 0); // 0 for lower 16b and 1 for upper 16b
		int imag0 = extractIQ(data0, 1);
	...
	for (unsigned int i=0; i<5; i++ ) {
		uint64_t addr0   = ulbfCoeffsAddr[ipNum] + ULBF_COEFFS_RAM2_OFFSET + (i*8);
		uint64_t addr1   = ulbfCoeffsAddr[ipNum] + ULBF_COEFFS_RAM2_OFFSET + ((i*8)+4);
		unsigned int data0 = Xil_In32(addr0);
		unsigned int data1 = Xil_In32(addr1);
		int real0 = extractIQ(data0, 0); // 0 for lower 16b and 1 for upper 16b
		int imag0 = extractIQ(data0, 1);
	...
	for (unsigned int i=0; i<5; i++ ) {
		uint64_t addr0   = ulbfCoeffsAddr[ipNum] + ULBF_COEFFS_RAM3_OFFSET + (i*8);
		uint64_t addr1   = ulbfCoeffsAddr[ipNum] + ULBF_COEFFS_RAM3_OFFSET + ((i*8)+4);
		unsigned int data0 = Xil_In32(addr0);
		unsigned int data1 = Xil_In32(addr1);
		int real0 = extractIQ(data0, 0); // 0 for lower 16b and 1 for upper 16b
		int imag0 = extractIQ(data0, 1);
	...
	for (unsigned int i=0; i<5; i++ ) {
		uint64_t addr0   = ulbfCoeffsAddr[ipNum] + ULBF_COEFFS_RAM4_OFFSET + (i*8);
		uint64_t addr1   = ulbfCoeffsAddr[ipNum] + ULBF_COEFFS_RAM4_OFFSET + ((i*8)+4);
		unsigned int data0 = Xil_In32(addr0);
		unsigned int data1 = Xil_In32(addr1);
		int real0 = extractIQ(data0, 0); // 0 for lower 16b and 1 for upper 16b
		int imag0 = extractIQ(data0, 1);
	...
	for (unsigned int i=0; i<5; i++ ) {
		uint64_t addr0   = ulbfCoeffsAddr[ipNum] + ULBF_COEFFS_RAM5_OFFSET + (i*8);
		uint64_t addr1   = ulbfCoeffsAddr[ipNum] + ULBF_COEFFS_RAM5_OFFSET + ((i*8)+4);
		unsigned int data0 = Xil_In32(addr0);
		unsigned int data1 = Xil_In32(addr1);
		int real0 = extractIQ(data0, 0); // 0 for lower 16b and 1 for upper 16b
		int imag0 = extractIQ(data0, 1);
	...
	for (unsigned int i=0; i<5; i++ ) {
		uint64_t addr0   = ulbfCoeffsAddr[ipNum] + ULBF_COEFFS_RAM6_OFFSET + (i*8);
		uint64_t addr1   = ulbfCoeffsAddr[ipNum] + ULBF_COEFFS_RAM6_OFFSET + ((i*8)+4);
		unsigned int data0 = Xil_In32(addr0);
		unsigned int data1 = Xil_In32(addr1);
		int real0 = extractIQ(data0, 0); // 0 for lower 16b and 1 for upper 16b
		int imag0 = extractIQ(data0, 1);
	...
	for (unsigned int i=0; i<5; i++ ) {
		uint64_t addr0   = ulbfCoeffsAddr[ipNum] + ULBF_COEFFS_RAM7_OFFSET + (i*8);
		uint64_t addr1   = ulbfCoeffsAddr[ipNum] + ULBF_COEFFS_RAM7_OFFSET + ((i*8)+4);
		unsigned int data0 = Xil_In32(addr0);
		unsigned int data1 = Xil_In32(addr1);
		int real0 = extractIQ(data0, 0); // 0 for lower 16b and 1 for upper 16b
		int imag0 = extractIQ(data0, 1);
