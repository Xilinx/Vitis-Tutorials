...
|-tclargs xcvc1902-vsva2197-2MP-e-S|The -tclargs option enables you to pass arguments for the Tcl script you run. The vck190_v1_0_xsa.tcl requires you to specify the device-package name.|
|-nodisp|No display|
|-sdx ||
...
|sw/src/boot/|Directory with files listed in file names provided as part of the qemu-args and pmuqemu-args options. It is the directory to generate components after the Linux image build.|
...
## Introduction to Custom Vitis Embedded Platform

A platform is the starting point of any Vitis design. Vitis applications are built on top of the platforms. AMD provides prebuilt base platforms at the [Xilinx download center](https://www.xilinx.com/support/download.html). The source code of these base platforms is available on the [Xilinx GitHub site](https://github.com/Xilinx/Vitis_Embedded_Platform_Source).
...
The hardware platform is the static portion of your hardware design. It includes the Xilinx support archive (XSA) file exported from the Vivado Design Suite. The hardware platform describes the platform hardware setup and the acceleration resources used by acceleration applications. For example, I/O interfaces, clocks, and AXI buses. The Vitis tool builds on top of the platform to connect PL kernels and the AI Engine design.
...
The software platform is the environment that runs software to control kernels for acceleration applications. It includes domain setup and boot component setup. The software platform in this beamforming tutorial has three domains: AI Engine, Linux, and bare-metal (standalone). Components to build the AI Engine and Linux domains are provided in the `sw_comp` folder.

The software platforms and final XPFM platform file are generated by the Xilinx Software Command-line Tool (XSCT) command. The XSCT tool is an interactive, scriptable command-line interface to the Vitis IDE.
...
The first operation this `vck_190_v1_0_xsa.tcl` script does is call the `create_project` function and create the Vivado `vck190_v1_0_vivado` project.

#### Create Block Design

The `vck_190_v1_0_xsa.tcl` script then calls the `dr.bd.tcl` script, which generates the platform block design. Open the block design in the Vivado project located at `hw/build/vck190_v1_0_vivado/vck190_v1_0.xpr`. There are five major components to the hardware platform: the AI Engine, CIPS, NoC (Network-on-Chip), Clocking Wizard IP, Clock Reset IPs, 16 AXI4-LITE SmartConnect interfaces, and a top-level AXI4-Lite SmartConnect (called ``ctrl_sm``).

![Platform Block Design](images/Beamforming_custom_BlockDiagram_2025_2.PNG)

Open the `dr.bd.tcl` file and review the comments. After the `Setup and Error Checking` commands, the main function of this file is called `create_root_design`.

#### Port Instantiation

The `create_root_design` function creates two block design interface ports called `SYS_CLK1_IN_0` and `ddr4_dimm1` for the system clock and DDR memory ports.  

#### AI Engine

Next, the `dr.bd.tcl` file creates an instance of the AI Engine and sets its properties. Most ports are empty (`{}`). Here, you have just instantiated the AI Engine interfaces in the platform. Later, the Vitis linker step connects the AI Engine to the PL kernels. At this step, the AI Engine `S00_AXI` slave interface configuration category is set to NoC.

#### AXI Debug Hub IP and Simulation Clock and Reset Generator IP

The AXI Debug Hub IP and the Simulation Clock and Reset Generator IP are then instantiated, and configured.

#### AXI SmartConnects

The `dr.bd.tcl` creates an instance of an AXI SmartConnect IP called `ctrl_sm`. This top-level AXI SmartConnect is configured with one clock, 16 master interfaces, and one slave interface. The script then instantiates 16 additional AXI SmartConnect IPs and connects them to the 16 master interfaces in the `ctrl_sm` later in the script.

#### AXI Verification IPs

Next, 16 instances of the AXI Verification IP are created, and their properties are set. Later, the 16 AXI SmartConnect IPs are connected to the 16 AXI Verification IPs.

#### Clock Infrastructure

Seven instances of the Processor Reset System IP and one instance of the Clocking Wizard are created. The Clocking Wizard is configured to generate seven output clocks (for example, `CLOCKOUT_REQUESTED_OUT_FREQUENCY`= 100 MHz, 250 MHz, 500 MHz, 400 MHz, 450 MHz, 250 MHz). It is recommended to also instantiate a Process Reset System IP is for each output clock to ensure proper clocking. Several clocks tied to the same reset can cause adverse affects such as cross-clock domain issues.

#### CIPS

Instantiate and configure the CIPS IP block. The system monitor is enabled with the use of the `CONFIG.SMON_*` properties.

#### NoC

Instantiate the NoC and set its properties. It has two AXI master interfaces, eight AXI slave interfaces, and 10 clocks. The NoC configuration is as follows:
...
After instantiating the major components, connect them with the `connect_bd_intf_net` commands.

#### Clock Connections

The first connection you creat connects the `SYS_CLK1_IN_0_1` port (created at the beginning of the `dr.bd.tcl` script) to the `SYS_CLK0_IN` input port of the Simulation Clock and Reset Generator IP.

Connect the `SYS_CLK0` pin of the Simulation Clock and Reset Generator IP to the `sys_clk0` pin of the NoC.

#### AXI SmartConnect Connections

Connect the 16 AXI Verification IP masters to the 16 AXI SmartConnect slaves. Then connect the 16 AXI SmartConnect to the 16 master interfaces in the top-level AXI SmartConnect (``sm_ctrl``). Connect the `sm_ctrl` to the master interface `M_AXI_FPD` on the CIPs. These connections allow any master AXI Verification IPs to access the CIPS slave connected to the `sm_ctrl` AXI SmartConnect.   

### CIPS and NoC Connections

Make the next set of connections between CIPS and the NoC. The CIPS connections have been added to the previous NoC interface table to illustrate the connections being created.  
...
Connect the output clocks of the Clocking Wizard to the rest of the design. First, connect the Clocking Wizard's output clock 6 (100 MHz) to the AXI Debug Hub IP and the NoC PL clock (`aclk5`). Synchronize the Processor Reset System #6 to this clock. Also connect the asynchronous reset of the AXI Debug Hub IP to the Processor Reset System #6.

Next, connect the Clocking Wizard's output clock 1 (250MHz) to the ``ctrl_sm``, the 16 AXI SmartConnects, 16 AXI Verification IPs, and the CIPS' `m_axi_fpd_aclk`. Synchronize the Processor Reset System #1 to this clock as well.

Synchronize the rest of the Processor Reset Systems (#2, 3, 4, and 5) to the Clocking Wizard's output clocks (clock output 2 = 500 MHz, clock output 3 = 400 MHz, clock output 4 = 450 MHz, and clock output 5 = 250 MHz).
...
The last set of connections connects the CIPS clock pins to the NoC clock pins. A summary table of the connections is as follows:
...
After making the connections, assign memory addresses in CIPS to AI Engine, AXI Debug Hub, 16 AXI SmartConnects,and NoC.
...
Every platform must declare at least one general purpose AXI master port (`M_AXI_GP`). For the 16 AXI SmartConnects, 15 `M_AXI_GP` interfaces are declared. Additionally, `M_AXI_NoC` is a control interface on the NoC. The Vitis linker uses these interfaces to connect the PL kernels to the platform.
...
Every platform must have at least one clock enabled in `PFM.CLOCK` property, and one clock must be set as default. Five clocks are declared and `clk_out1` is the default.
...
Some platforms require post-link Tcl commands to complete platform creation. The beamforming design does not require any post link Tcl commands, so this script is empty. Use it if you modify the beamforming platform and add any post link Tcl commands to the platform Vivado project.

#### Timing Closure

After you create a Vivado design, Vivado then runs implementation: placement, routing, and physical optimization.

* Placement places specified ports and logic cells onto device resources.
...
Large platforms often require Tcl commands before and after these steps to guide the Vivado tool to generate a timing-closed implementation.
...
The `post_route.tcl` and the `post_physopts.tcl` scripts are empty because this tutorial does not require Tcl commands after routing and physical optimization. Use them if you want to modify this tutorial.

#### Emulation Setup

The `vck_190_v1_0_xsa.tcl` script sets up the platform for hardware emulation. For emulation-capable platforms, CIPS and NoC must use TLM (transaction-level modeling) as the simulation model.  
...
The next commands in  the `vck_190_v1_0_xsa.tcl` script finalize and wrap up the Vivado project creation process by updating the compiler order, assigning the block design addresses, and validating the block design.

#### Export Hardware XSA*

Lastly, the `vck_190_v1_0_xsa.tcl` script generates the files necessary to support the block design with the `generate_target` command and writes the pre-synthesis expandable XSA with the `write_hw_platform` command.

### Software Platform

After creating the hardware platform (XSA) with the AI Engine IP through Vivado, import this XSA into the Vitis environment and create the software platform. Then add the aie_runtime, Linux, and standalone (bare-metal) domains into the software platform. The domains are configured with prebuilt PetaLinux software components. Review Steps 2 and 3 of the [Versal Custom Platform Creation Tutorial](https://github.com/Xilinx/Vitis-Tutorials/blob/2025.2/Vitis_Platform_Creation/Design_Tutorials/03_Edge_VCK190/README.md) to learn how to create the Petalinux software components from scratch.  
...
The BIF file describes the boot components and their properties for Bootgen to generate the ``boot.bin`` file. Provide a BIF file so the application build process can package the boot image.

##### Boot Directory

The boot directory includes all the files described in the BIF file. The contents of the image directory are copied into the FAT32 partition of the final SD card image. In the beamforming example, the boot directory contains `bl31.elf`, `u-boot.elf`, and `system.dtb` generated by PetaLinux. Later, Vitis compiler (``v++``) looks for files in the boot directory and replaces the placeholders with real file names and paths. It then calls Bootgen to generate the ``boot.bin`` file.
...
Finally, the software platform is generated with the `platform generate` command. The final generated custom platform is in `build/output/vck190_v1_0/export/vck190_v1_0/vck190_v1_0.xpfm`. This platform is used in later modules. The Vitis tool builds on top of it.
...
* [Software Command-Line Tool (XSCT)](https://docs.amd.com/r/en-US/ug1400-vitis-embedded)
