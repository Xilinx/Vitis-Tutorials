...
public:
  ...
  xf::dsp::aie::func_approx::func_approx_graph<TT_DATA,TP_COARSE_BITS,TP_FINE_BITS,
                                               ...
                                               TP_RND,TP_SAT> sin_dsp;

  // Constructor:
  sin_lib_graph() : LUT_init({initialize_sin_LUT<TT_DATA,LUT_SIZE,TP_COARSE_BITS,TP_DOMAIN_MODE>()}),
                    sin_dsp(LUT_init)
  {
    // Initialize graph:
    // Throughput:     1740 MB/sec or ~420 Msps
    // Total cycles:   1478
    connect<>( sig_i,           sin_dsp.in[0] );
    connect<>( sin_dsp.out[0],  sig_o );
    repetition_count(sin_dsp.m_kernel[0]) = REPEAT;
  }
};
