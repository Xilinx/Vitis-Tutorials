...
% Model:
% --> assume input had been reduced to range [1,4):
tmp = 2.5+1.5*(1-2*single(rand(Nsamp,Niter)));
sig_i = sort(tmp,1);
sig_o = sqrt(sig_i);
...
vfs_model = vfs.aieGraph(input_file="sqrt_lib_app.cpp",...
...
figure;
subplot(3,1,1); plot(data_i,'b.-'); title('sqrt() Input');
subplot(3,1,2); plot(data_o,'b.-'); hold on; plot(vfs_o,'r.'); hold off; title('sqrt() Output');
subplot(3,1,3); plot(err_o,'k.-'); ylabel('error'); title('sqrt() Error'); 
yline(1.5e-6,'r'); yline(-1.5e-6,'r');

if (max(abs(err_o)) < 1.5e-6)
  fprintf(1,'--- PASSED ---\n');
else
  fprintf(1,'*** FAILED ***\n');
end
