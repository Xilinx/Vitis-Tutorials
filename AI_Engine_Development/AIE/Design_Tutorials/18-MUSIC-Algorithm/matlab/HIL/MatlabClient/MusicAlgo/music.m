...
function data = music (A, sysCfg, musiCfg)
...
[data] = computeNoiseSS (data, sysCfg, musiCfg);
...
[data] = computeDoa(data, sysCfg, musiCfg);
doaAngles = ['\n          MATLAB-Estimated DOA Angles (Degrees) =  [', repmat('%g, ', 1, numel(data.doaEst)-1), '%g]\n'];
fprintf(doaAngles,  data.doaEst);
