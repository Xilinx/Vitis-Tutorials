...
function [data] = svdOneSidedJacobi(data, jacobiRotation, sysCfg, musiCfg)  
   % Perform one-sided Jacobi for Matrix data.R
   nUla         = sysCfg.nUla;
   V            = single(eye(nUla)); % identity matrix
   W            = single(data.R);
   nIter        = musiCfg.svdNumIter;
   ...
   [U, S, s]  = computeSingularValues (W, sysCfg); 
...
% -------------------------
function [U, S, s]  = computeSingularValues (W, sysCfg)
