...
extern void m16_ssr8_permute_fb_o_wrapper( TT_DUT::TT_STREAM sig_i[TT_DUT::SSR],
...
int main( void )
{
  ...
  for (unsigned ii=0; ii < NSAMP/2; ii++) {
    ...
    for (unsigned ss=0; ss < TT_DUT::SSR; ss++) {
    ...
    // Run DUT:
    m16_ssr8_permute_fb_o_wrapper( sig_i, sig_o );
