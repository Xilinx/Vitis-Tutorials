...
extern void pfa1008_transpose1_wrapper( TT_STREAM sig_i[SSR_IO], TT_STREAM sig_o[SSR_IO] );
...
int main( void )
{
  ...
  for (int ss=0; ss < NTRANSFORM*NFFT/8+LATENCY; ss++) {
    pfa1008_transpose1_wrapper( sig_i, sig_o );
  }

  cout << "Done pfa1008_transpose1_wrapper()" << endl;
