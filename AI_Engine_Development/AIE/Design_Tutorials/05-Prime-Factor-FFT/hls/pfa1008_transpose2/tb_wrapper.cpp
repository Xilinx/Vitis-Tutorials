...
extern void pfa1008_transpose2_wrapper( TT_STREAM sig_i[SSR_IO], TT_STREAM sig_o[SSR_IO] );
...
int main( void )
{
  ...
  // Write zeros to manage startup latency of 1+NFFT/8 cycles:
  int LATENCY = NFFT/8+1;
  ...
  for (int ss=0; ss < NTRANSFORM*NFFT/8+LATENCY; ss++) {
    pfa1008_transpose2_wrapper( sig_i, sig_o );
  }

  cout << "Done pfa1008_transpose2_wrapper()" << endl;
